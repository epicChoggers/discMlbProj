...
export const Leaderboard = ({ gamePk }: LeaderboardProps) => {
  ...
  useEffect(() => {
    const loadLeaderboard = async () => {
      ...
      try {
        const leaderboardData = await leaderboardService.getLeaderboard(gamePk, 10)
    ...
    const subscription = leaderboardService.subscribeToLeaderboard(gamePk, (newLeaderboard) => {
