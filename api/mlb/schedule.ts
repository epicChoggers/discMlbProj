...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { teamId, startDate, endDate, hydrate } = req.query

    // Build the MLB API URL
    const params = new URLSearchParams()
    params.append('sportId', '1') // MLB sport ID
    ...
    const mlbUrl = `${MLB_BASE_URL}/schedule?${params.toString()}`
    
    console.log('Fetching MLB schedule:', mlbUrl)
