...
-- Drop existing triggers (they depend on tables)
...
DROP TRIGGER IF EXISTS update_cached_game_states_updated_at ON cached_game_states CASCADE;
DROP TRIGGER IF EXISTS update_cached_at_bats_updated_at ON cached_at_bats CASCADE;

-- Drop existing functions
DROP FUNCTION IF EXISTS cleanup_old_cache_entries() CASCADE;
DROP FUNCTION IF EXISTS get_leaderboard_data(INTEGER, INTEGER) CASCADE;
DROP FUNCTION IF EXISTS update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS handle_new_user() CASCADE;

-- Drop existing tables if they exist (in correct order due to foreign keys)
DROP TABLE IF EXISTS prediction_resolution_log CASCADE;
DROP TABLE IF EXISTS game_sync_log CASCADE;
...
CREATE INDEX idx_at_bat_predictions_at_bat_index ON at_bat_predictions(at_bat_index);
...
CREATE INDEX idx_at_bat_predictions_is_correct ON at_bat_predictions(is_correct);
CREATE INDEX idx_at_bat_predictions_points_earned ON at_bat_predictions(points_earned);
...
CREATE INDEX idx_cached_at_bats_created_at ON cached_at_bats(created_at);
...
CREATE INDEX idx_game_sync_log_game_pk ON game_sync_log(game_pk);
CREATE INDEX idx_game_sync_log_created_at ON game_sync_log(created_at);
CREATE INDEX idx_game_sync_log_status ON game_sync_log(status);

-- Indexes for prediction_resolution_log
CREATE INDEX idx_prediction_resolution_log_game_pk ON prediction_resolution_log(game_pk);
CREATE INDEX idx_prediction_resolution_log_at_bat_index ON prediction_resolution_log(at_bat_index);
CREATE INDEX idx_prediction_resolution_log_created_at ON prediction_resolution_log(created_at);
...
COMMENT ON TABLE user_profiles IS 'Discord OAuth user profiles with avatar and metadata';
COMMENT ON TABLE at_bat_predictions IS 'User predictions for MLB at-bats with scoring and streak tracking';
COMMENT ON TABLE cached_game_states IS 'Cached MLB game state data for performance';
COMMENT ON TABLE cached_at_bats IS 'Cached MLB at-bat data for prediction resolution';
COMMENT ON TABLE system_health IS 'System health monitoring and performance metrics';
COMMENT ON TABLE sync_logs IS 'Data synchronization tracking';
COMMENT ON TABLE game_sync_log IS 'MLB API synchronization events';
COMMENT ON TABLE prediction_resolution_log IS 'Prediction resolution tracking';

COMMENT ON COLUMN user_profiles.discord_id IS 'Discord user ID from OAuth metadata';
COMMENT ON COLUMN at_bat_predictions.streak_count IS 'Current streak count when prediction was made';
COMMENT ON COLUMN at_bat_predictions.streak_bonus IS 'Bonus points earned from streak';
